...
public class CTabFolderNext extends Composite {
	...
	/* item management */
	CTabFolderNextRenderer renderer;
	CTabItemNext items[] = new CTabItemNext[0];
	...
	CTabFolderNext2Listener[] folderListeners = new CTabFolderNext2Listener[0];
	// support for deprecated listener mechanism
	CTabFolderNextListener[] tabListeners = new CTabFolderNextListener[0];
	...
	static final int LEFT_RIGHT_TOP_INDENT = 10;
 ...
 * @see #getStyle()
 */
public CTabFolderNext(Composite parent, int style) {
	super(parent, checkStyle (parent, style));
	init(style);
}

void init(int style) {
	super.setLayout(new CTabFolderNextLayout());
...
//	showMax = (style2 & SWT.MAX) != 0; - conflicts with SWT.BOTTOM
	...
	renderer = new CTabFolderNextRenderer(this);
 ...
 * </ul>
 *
 * @see CTabFolderNext2Listener
 * @see #removeCTabFolder2Listener(CTabFolderNext2Listener)
 *
 * @since 3.0
 */
public void addCTabFolder2Listener(CTabFolderNext2Listener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	// add to array
	CTabFolderNext2Listener[] newListeners = new CTabFolderNext2Listener[folderListeners.length + 1];
 ...
 * </ul>
 *
 * @see CTabFolderNextListener
 * @see #removeCTabFolderListener(CTabFolderNextListener)
 *
 * @deprecated use addCTabFolder2Listener(CTabFolder2Listener)
 */
@Deprecated
public void addCTabFolderListener(CTabFolderNextListener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	// add to array
	CTabFolderNextListener[] newTabListeners = new CTabFolderNextListener[tabListeners.length + 1];
...
Rectangle[] computeControlBounds (Point size, boolean[][] position) {
	...
	Rectangle trim = renderer.computeTrim(CTabFolderNextRenderer.PART_BORDER, SWT.NONE, 0, 0, 0, 0);
	int borderRight = (tabPosition == LEFT) ? size.x - getTabWidth() : trim.width + trim.x;
	...
	if(tabPosition != LEFT && tabPosition != RIGHT) {
		//in left and right orientation the tabs start below the controls and do not need to be used
	...
	Rectangle bodyTrim = renderer.computeTrim(CTabFolderNextRenderer.PART_BODY, SWT.NONE, 0, 0, 0, 0);
	...
	if (showChevron) {
		...
		if (lastIndex != -1) {
			CTabItemNext lastItem = items[lastIndex];
...
public Rectangle computeTrim (int x, int y, int width, int height) {
	checkWidget();
	Rectangle trim =  renderer.computeTrim(CTabFolderNextRenderer.PART_BODY, SWT.NONE, x, y, width, height);
...
void createItem (CTabItemNext item, int index) {
	if (0 > index || index > getItemCount ())SWT.error (SWT.ERROR_INVALID_RANGE);
	item.parent = this;
	CTabItemNext[] newItems = new CTabItemNext [items.length + 1];
...
void destroyItem (CTabItemNext item) {
	if (inDispose) return;
	int index = indexOf(item);
	if (index == -1) return;

	if (items.length == 1) {
		items = new CTabItemNext[0];
		priority = new int[0];
		firstIndex = -1;
		selectedIndex = -1;
...
public Rectangle getClientArea() {
	...
	Rectangle trim = renderer.computeTrim(CTabFolderNextRenderer.PART_BODY, SWT.FILL, 0, 0, 0, 0);
	Point size = getSize();
	int wrapHeight = getWrappedHeight(size);
	...
	return new Rectangle(-trim.x, -trim.y, size.x - trim.width, size.y - trim.height);
}
 ...
 * </ul>
 */
public CTabItemNext getItem (int index) {
	/*
 ...
 *	</ul>
 */
public CTabItemNext getItem (Point pt) {
	/*
	 * This call is intentionally commented out, to allow this getter method to be
	 * called from a thread which is different from one that created the widget.
	 */
	...
	for (int element : priority) {
		CTabItemNext item = items[element];
 ...
 *	</ul>
 */
public CTabItemNext [] getItems() {
	/*
	 * This call is intentionally commented out, to allow this getter method to be
	 * called from a thread which is different from one that created the widget.
	 */
	//checkWidget();
	CTabItemNext[] tabItems = new CTabItemNext [items.length];
 ...
 *	</ul>
 *
 * @see #setRenderer(CTabFolderNextRenderer)
 * @see CTabFolderNextRenderer
 *
 * @since 3.6
 */
public CTabFolderNextRenderer getRenderer() {
	checkWidget();
	return renderer;
}
...
int getRightItemEdge (GC gc){
	Rectangle trim = renderer.computeTrim(CTabFolderNextRenderer.PART_BORDER, SWT.NONE, 0, 0, 0, 0);
	if(tabPosition == LEFT) {
		return getTabWidth() + (trim.width + trim.x);
	}else if(tabPosition == RIGHT) {
		return getSize().x - (trim.width + trim.x);
	}
 ...
 *	</ul>
 */
public CTabItemNext getSelection() {
	/*
	 * This call is intentionally commented out, to allow this getter method to be
	 * called from a thread which is different from one that created the widget.
	 */
 ...
 * </ul>
 */
public int indexOf(CTabItemNext item) {
	checkWidget();
	if (item == null) {
		SWT.error(SWT.ERROR_NULL_ARGUMENT);
	}
			...
			if (!mru) {
				...
				if (current + offset >= 0 && current + offset < idx){
					...
					if (showChevron) {
						...
						CTabFolderNextEvent e = new CTabFolderNextEvent(this);
						e.widget = this;
						e.time = event.time;
						e.x = chevronRect.x;
						e.y = chevronRect.y;
						...
						e.doit = true;
						for (CTabFolderNext2Listener folderListener : folderListeners) {
...
void onDispose(Event event) {
	...
	if (folderListeners.length != 0) folderListeners = new CTabFolderNext2Listener[0];
	if (tabListeners.length != 0) tabListeners = new CTabFolderNextListener[0];
}
...
void onMenuDetect(Event event) {
	if (event.detail == SWT.MENU_KEYBOARD) {
		if (selectedIndex != -1) {
			CTabItemNext item = items[selectedIndex];
			...
			if (!rect.contains(event.x, event.y)) {
				...
				Rectangle closeTrim = renderer.computeTrim(CTabFolderNextRenderer.PART_CLOSE_BUTTON, SWT.NONE, 0, 0, 0, 0);
	...
	switch (event.type) {
		...
		case SWT.MouseDown: {
								...
								for (Control control : controls) {
									...
									do {
										if (temp.equals(control)) {
											...
											temp = temp.getParent();
											if (temp == null || temp.equals(CTabFolderNext.this)) break;
										}
									} while (!reschedule);
									if (reschedule) break;
								}
			...
			if (single) {
				...
				for (CTabItemNext tabItem : items) {
		...
		case SWT.MouseUp: {
			...
			if (item != null) {
				if (item.closeRect.contains(x,y)) {
					...
					e.item = item;
					e.doit = true;
					for (CTabFolderNext2Listener listener : folderListeners) {
						listener.close(e);
					}
					for (CTabFolderNextListener listener : tabListeners) {
					...
					if (!isDisposed() && item.isDisposed()) {
						...
						CTabItemNext nextItem = getItem(pt);
						if (nextItem != null) {
...
void onPaint(Event event) {
	...
	if (oldFont == null || !oldFont.equals(font)) {
		...
		oldFont = font;
		if (!updateTabHeight(false) || !updateTabWidth(false)) {
			updateItems();
			redraw();
			return;
		}
...
//}
	...
	renderer.draw(CTabFolderNextRenderer.PART_BODY, SWT.BACKGROUND | SWT.FOREGROUND, bodyRect, gc);

	gc.setFont(gcFont);
	gc.setForeground(gcForeground);
	gc.setBackground(gcBackground);

	renderer.draw(CTabFolderNextRenderer.PART_HEADER, SWT.BACKGROUND | SWT.FOREGROUND, bodyRect, gc);
...
void onResize(Event event) {
	...
	if (oldSize == null) {
		...
		if (onBottom && size.y != oldSize.y) {
			...
			Rectangle trim = renderer.computeTrim(CTabFolderNextRenderer.PART_BODY, SWT.NONE, 0, 0, 0, 0);
 ...
 * </ul>
 *
 * @see #addCTabFolder2Listener(CTabFolderNext2Listener)
 *
 * @since 3.0
 */
public void removeCTabFolder2Listener(CTabFolderNext2Listener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	...
	if (folderListeners.length == 1) {
		folderListeners = new CTabFolderNext2Listener[0];
		return;
	}
	CTabFolderNext2Listener[] newTabListeners = new CTabFolderNext2Listener[folderListeners.length - 1];
 ...
 */
@Deprecated
public void removeCTabFolderListener(CTabFolderNextListener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	...
	if (tabListeners.length == 1) {
		tabListeners = new CTabFolderNextListener[0];
		return;
	}
	CTabFolderNextListener[] newTabListeners = new CTabFolderNextListener[tabListeners.length - 1];
...
void updateButtons() {
	...
	if (showMax) {
		...
		if (maxItem == null) {
			...
			if (maxImage == null) {
				maxImage = createButtonImage(display, CTabFolderNextRenderer.PART_MAX_BUTTON);
			}
	...
	if (showMin) {
		...
		if (minItem == null) {
			...
			if (minImage == null) {
				minImage = createButtonImage(display, CTabFolderNextRenderer.PART_MIN_BUTTON);
			}
...
private void updateChevronImage(boolean styleChange) {
	...
	if (chevronImage != null) chevronImage.dispose();
	chevronImage = createButtonImage(getDisplay(), CTabFolderNextRenderer.PART_CHEVRON_BUTTON);
...
public boolean setFocus () {
	...
	*/
	...
	if (fixFocus) {
		CTabItemNext item = getSelection();
		if (item != null) {
 ...
 * </ul>
 */
public void setInsertMark(CTabItemNext item, boolean after) {
	checkWidget();
}
/**
 * Display an insert marker before or after the specified tab item.
...
boolean setItemLocation(GC gc) {
	...
	int xx = onRight ? Math.max(borderRight, size.x - borderRight - tabWidth) : borderLeft;
	Point closeButtonSize = renderer.computeSize(CTabFolderNextRenderer.PART_CLOSE_BUTTON, 0, gc, SWT.DEFAULT, SWT.DEFAULT);
	int leftItemEdge = getLeftItemEdge(gc, CTabFolderNextRenderer.PART_BORDER);
	int rightItemEdge = getRightItemEdge(gc);
	int topItemEdge = getTopItemEdge(gc, CTabFolderNextRenderer.PART_BORDER);
	if (single) {
		int defaultX = getDisplay().getBounds().width + 10; // off screen
		if(tabPosition == LEFT || tabPosition == RIGHT &&  controls != null && controls.length > 0) {
			//we have controls place the first tab below them
			boolean[][] positions = new boolean[1][];
			Rectangle[] rects = computeControlBounds(new Point(0, 0), positions);
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (i == selectedIndex) {
				...
				item.x = onRight ? xx + renderer.computeTrim(i, SWT.NONE, 0, 0, 0, 0).x : leftItemEdge;
				item.y = y;
				item.showing = true;
				if (showClose || item.showClose) {
					item.closeRect.x = item.x- renderer.computeTrim(i, SWT.NONE, 0, 0, 0, 0).x;
					item.closeRect.y = item.y + (item.height - closeButtonSize.y)/2;
				}
		...
		for (int i = 0; i < priority.length; i++) {
			CTabItemNext item = items[priority[i]];
			height += item.height;
		...
		if(controls != null && controls.length > 0 && controls[0].getVisible()) {
			//we have controls place the first tab below them
			boolean[][] positions = new boolean[1][];
			Rectangle[] rects = computeControlBounds(new Point(0, 0), positions);
			y = rects[0].y + rects[0].height;
		}else {
			y += LEFT_RIGHT_TOP_INDENT;  //move the first item a bit down to make it nicer drawn
		}
		for (int i = 0; i < items.length; i++) {
			CTabItemNext item = items[i];
			if (!item.showing) {
				...
				if (item.x != xx || item.y != y) changed = true;
				item.x = xx;
				item.y = y;
				item.width = tabWidth;
				int state = SWT.NONE;
		...
		int x = getLeftItemEdge(gc, CTabFolderNextRenderer.PART_HEADER);
		int defaultX = getDisplay().getBounds().width + 10; // off screen
		firstIndex = items.length - 1;
		for (int i = 0; i < items.length; i++) {
			CTabItemNext item = items[i];
			if (!item.showing) {
				...
				Rectangle edgeTrim = renderer.computeTrim(i, state, 0, 0, 0, 0);
				item.closeRect.x = item.x + item.width  - (edgeTrim.width + edgeTrim.x) - closeButtonSize.x;
				item.closeRect.y = item.y + (tabHeight - closeButtonSize.y)/2;
				x = x + item.width;
...
/*public*/ void setItemOrder (int[] indices) {
	...
	CTabItemNext[] temp = new CTabItemNext[items.length];
...
boolean setItemSize(GC gc) {
	...
	if (single) {
		...
		if (selectedIndex != -1) {
			CTabItemNext tab = items[selectedIndex];
			Point itemSize = renderer.computeSize(selectedIndex, SWT.SELECTED, gc, SWT.DEFAULT, SWT.DEFAULT);
			...
			if(tabPosition == LEFT || tabPosition == RIGHT) {
				...
				width = Math.min(width, getRightItemEdge(gc) - getLeftItemEdge(gc, CTabFolderNextRenderer.PART_BORDER));
			}

			if (tab.height != height || tab.width != width) {
				changed = true;
				...
				if (showClose || tab.showClose) {
					Point closeSize = renderer.computeSize(CTabFolderNextRenderer.PART_CLOSE_BUTTON, SWT.SELECTED, gc, SWT.DEFAULT, SWT.DEFAULT);
	...
	if(tabPosition == LEFT || tabPosition == RIGHT) {
		return setLeftRightItemSize(gc);
	}

	int[] widths;
	int tabAreaWidth = Math.max(0, getRightItemEdge(gc) - getLeftItemEdge(gc, CTabFolderNextRenderer.PART_BORDER));
	// First, try the minimum tab size at full compression.
	int minWidth = 0;
	int[] minWidths = new int[items.length];
	for (int element : priority) {
		int index = element;
		int state = CTabFolderNextRenderer.MINIMUM_SIZE;
	...
	for (int i = 0; i < items.length; i++) {
		...
		if ((showClose || tab.showClose)
			...
			Point closeSize = renderer.computeSize(CTabFolderNextRenderer.PART_CLOSE_BUTTON, SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT);
			tab.closeRect.width = closeSize.x;
			tab.closeRect.height = closeSize.y;
		}
	}
...
private boolean setLeftRightItemSize(GC gc) {
	boolean changed = false;
	int tabAreaHeight = Math.max(0, getBottomItemEdge(gc) - getTopItemEdge(gc, CTabFolderNextRenderer.PART_BORDER));
	// First, try the minimum tab size at full compression.
	int minHeight = 0;
	...
	for (int element : priority) {
		...
		int state = CTabFolderNextRenderer.MINIMUM_SIZE;
		if (index == selectedIndex) state |= SWT.SELECTED;
		heights[index] = renderer.computeSize(index, state, gc, SWT.DEFAULT, SWT.DEFAULT).y;
		minHeight += heights[index];
		if (minHeight > tabAreaHeight) break;
	}

	if (minHeight > tabAreaHeight) {
		// full compression required and a chevron
		showChevron = chevronVisible && items.length > 1;
		if (showChevron && !(null != controls && controls.length > 0)) {
			//if we have controls the chevron does not need additional space
			ToolBar chevron = getChevron();
			tabAreaHeight -= chevron.computeSize(SWT.DEFAULT, SWT.DEFAULT).y;
			chevron.setVisible(true);
		}
		int index = selectedIndex != -1 ? selectedIndex : 0;
		if (tabAreaHeight < heights[index]) {
			heights[index] = Math.max(0, tabAreaHeight);
		}
	}
	...
	for (int i = 0; i < items.length; i++) {
		...
		if (tab.height != height || tab.width != tabWidth) {
			changed = true;
			tab.shortenedText = null;
			tab.shortenedTextWidth = 0;
			tab.height = height;
...
public void setMaximized(boolean maximize) {
	...
	if (minMaxTb != null && maxItem != null) {
		if (maxImage != null) maxImage.dispose();
		maxImage = createButtonImage(getDisplay(), CTabFolderNextRenderer.PART_MAX_BUTTON);
		maxItem.setImage(maxImage);
		maxItem.setToolTipText(maximized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Maximize")); //$NON-NLS-1$ //$NON-NLS-2$
	}
}
...
public void setMinimized(boolean minimize) {
	...
	if (minMaxTb != null && minItem != null) {
		if (minImage != null) minImage.dispose();
		minImage = createButtonImage(getDisplay(), CTabFolderNextRenderer.PART_MIN_BUTTON);
		minItem.setImage(minImage);
		minItem.setToolTipText(minimized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Minimize")); //$NON-NLS-1$ //$NON-NLS-2$
	}
}
 ...
 * @since 3.6
 */
public void setRenderer(CTabFolderNextRenderer renderer) {
	checkWidget();
	...
	if (useDefaultRenderer) renderer = new CTabFolderNextRenderer(this);
 ...
 * </ul>
 */
public void setSelection(CTabItemNext item) {
	checkWidget();
...
public void setSelection(int index) {
	...
	CTabItemNext selection = items[index];
	if (selectedIndex == index) {
		showItem(selection);
		return;
	}
 ...
 * </ul>
 *
 * @see CTabFolderNext#showSelection()
 *
 * @since 2.0
 */
public void showItem (CTabItemNext item) {
	checkWidget();
	if (item == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	...
	for (CTabItemNext tab : items) {
		...
		item.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				int index = indexOf((CTabItemNext)menuItem.getData(id));
				CTabFolderNext.this.setSelection(index, true);
			}
		});
	}
	int x = rect.x;
 ...
 * </ul>
 *
 * @see CTabFolderNext#showItem(CTabItemNext)
 *
 * @since 2.0
 */
...
boolean updateItems (int showIndex) {
	...
	if (!single && !mru && showIndex != -1) {
		...
		if (priority[0] < showIndex) {
			int maxWidth = getRightItemEdge(gc) - getLeftItemEdge(gc, CTabFolderNextRenderer.PART_BORDER);
			int width = 0;
...
boolean updateTabHeight(boolean force){
	...
	GC gc = new GC(this);
	tabHeight = renderer.computeSize(CTabFolderNextRenderer.PART_HEADER, SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT).y;
	if(!(tabPosition == LEFT || tabPosition == RIGHT)) {
		if (fixedTabHeight == SWT.DEFAULT && controls != null && controls.length > 0) {
			for (int i = 0; i < controls.length; i++) {
				if ((controlAlignments[i] & SWT.WRAP) == 0 && !controls[i].isDisposed() && controls[i].getVisible()) {
					...
					topHeight +=  renderer.computeTrim(CTabFolderNextRenderer.PART_HEADER, SWT.NONE, 0,0,0,0).height + 1;
					tabHeight = Math.max(topHeight, tabHeight);
				}
			}
		}
	}
//	else if (fixedTabHeight == SWT.DEFAULT && items != null && items.length > 0) {
//		tabHeight = renderer.computeSize(0, SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT).y;
//	} else {
//		tabHeight = fixedTabHeight;
//	}
	gc.dispose();
...
boolean updateTabWidth(boolean force){
	int oldWidth = tabWidth;
	GC gc = new GC(this);
	tabWidth = renderer.computeSize(CTabFolderNextRenderer.PART_HEADER, SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT).x;
	gc.dispose();
	if (fixedTabWidth == SWT.DEFAULT && items != null) {
		GC tabWidthGC = new GC(this);
		for (int i = 0; i < items.length; i++) {
			//we compute the size in selected state as in selected we need the most width
			tabWidth = Math.max(tabWidth, renderer.computeSize(i, SWT.SELECTED, tabWidthGC, SWT.DEFAULT, SWT.DEFAULT).x);
		}
		tabWidthGC.dispose();
	}

	if(controls != null) {
		//if we have controls check if they need more space then the tabs
		tabWidth = Math.max(tabWidth, getControllAreaWidth());
	}

	if (!force && tabWidth == oldWidth) return false;
	oldSize = null;
	return true;
}

int getControllAreaWidth() {
	boolean[][] positions = new boolean[1][];
	Rectangle[] rects = computeControlBounds(new Point(0, 0), positions);
	...
	for (int i = 0; i < rects.length; i++) {
		Rectangle trim = renderer.computeTrim(CTabFolderNextRenderer.PART_CHEVRON_BUTTON, SWT.NONE, 0,0,0,0);
		left = Math.min(left, rects[i].x + trim.x);
		right = Math.max(right, rects[i].x + trim.x + rects[i].width+ trim.width);
	}
	return right - left;
}
			...
			if (!control.isDisposed()) {
				if (hovering) {
					...
					if (wrapped || gradientColors == null) {
						...
						renderer.draw(CTabFolderNextRenderer.PART_BACKGROUND, 0, bounds, gc);
						gc.dispose();
...
String _getToolTip(int x, int y) {
	CTabItemNext item = getItem(new Point (x, y));

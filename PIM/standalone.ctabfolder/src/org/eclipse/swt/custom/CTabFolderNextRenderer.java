 ...
 *     Alois Zoitl <alois.zoitl@gmx.at> - Bug 58945
 *******************************************************************************/
 ...
 * @since 3.6
 */
public class CTabFolderNextRenderer {

	protected CTabFolderNext parent;
	 ...
	 * @see Widget#getStyle
	 */
	protected CTabFolderNextRenderer(CTabFolderNext parent) {
		...
		switch (part) {
			case PART_HEADER:
				if (parent.fixedTabHeight != SWT.DEFAULT) {
					...
					CTabItemNext[] items = parent.items;
					if (items.length == 0) {
						height = gc.textExtent("Default", FLAGS).y + ITEM_TOP_MARGIN + ITEM_BOTTOM_MARGIN; //$NON-NLS-1$
					} else if(isLeftOrRightTabPosition()){
						if(parent.chevronVisible || (parent.controls != null && parent.controls.length != 0)) {
							height += BUTTON_SIZE + ITEM_TOP_MARGIN + ITEM_BOTTOM_MARGIN;
						}
						for (int i=0; i < items.length; i++) {
							height += computeSize(i, SWT.NONE, gc, wHint, hHint).y;
						}
					} else {
						for (int i=0; i < items.length; i++) {
			...
			default:
				if (0 <= part && part < parent.getItemCount()) {
					updateCurves();
					CTabItemNext item = parent.items[part];
	...
	protected Rectangle computeTrim (int part, int state, int x, int y, int width, int height) {
		...
		int borderTop = (parent.tabPosition != CTabFolderNext.TOP) ? borderLeft : 0;
		...
		switch (part) {
			case PART_BODY:
				...
				if (parent.minimized) {
					y = (parent.tabPosition != CTabFolderNext.TOP) ? y - borderTop : y - highlight_header - tabHeight - borderTop;
					height = borderTop + borderBottom;
					if(!isLeftOrRightTabPosition()) {
						height += tabHeight + highlight_header;
					} else {
						height += tabHeight * parent.items.length + parent.controls[0].computeSize(SWT.DEFAULT, SWT.DEFAULT).y;
						width = tabWidth + borderLeft + borderRight + 2*marginWidth + 2*highlight_margin;
					}
					if(parent.tabPosition == CTabFolderNext.LEFT){
						x = x - tabWidth - highlight_header + highlight_margin;
					}
				} else {
					height = height + borderTop + borderBottom + 2 * marginHeight  + highlight_margin;
					if(!isLeftOrRightTabPosition()) {
						height += tabHeight + highlight_header;
					}
					...
					case CTabFolderNext.LEFT:
						x = x - tabWidth - highlight_header + highlight_margin;
						//y, width, and height are equal for LEFT and right therefore no break here
					case CTabFolderNext.RIGHT:
						y = y - marginHeight - highlight_margin - borderTop;
						width += tabWidth + highlight_header - highlight_margin;
	...
	void drawBody(GC gc, Rectangle bounds, int state) {
		...
		if (!parent.minimized){
			...
			if (highlightMargin > 0) {
				drawHighlightMargin(gc, size, borderLeft, borderRight, borderTop, borderBottom,
						highlightHeader, highlightMargin);
			}
			//Draw client area
			if ((parent.getStyle() & SWT.NO_BACKGROUND) != 0) {
				drawClientArea(gc, size, borderLeft, borderRight, borderTop, borderBottom,
						highlightHeader, highlightMargin);
			}
		} else {
			if ((parent.getStyle() & SWT.NO_BACKGROUND) != 0) {
				int tabArea = borderLeft + highlightHeader;
				gc.setBackground(parent.getParent().getBackground());
				if(isLeftOrRightTabPosition()) {
					tabArea += parent.tabWidth;
					if (size.x > tabArea) {
						gc.fillRectangle(0, tabArea, size.x, size.y - tabArea);
					}
				} else {
					tabArea += parent.tabHeight;
					if (size.y > tabArea) {
						gc.fillRectangle(tabArea, 0, size.x - tabArea, size.y);
					}
				}
			}
		}
		...
		if (borderLeft > 0) {
			gc.setForeground(parent.getDisplay().getSystemColor(BORDER1_COLOR));
			int x1 = (parent.tabPosition == CTabFolderNext.LEFT) ? borderLeft + parent.tabWidth: borderLeft - 1;
			int x2 = parent.onRight ? size.x - borderRight - parent.tabWidth - 1 : size.x - borderRight;
			int y1 = parent.tabPosition == CTabFolderNext.TOP ? borderTop + parent.tabHeight : borderTop - 1;
			int y2 = parent.onBottom ? size.y - parent.tabHeight - borderBottom - 1 : size.y - borderBottom;
			if(parent.tabPosition != CTabFolderNext.LEFT) {
				gc.drawLine(x1, y1, x1, y2); // left
			}
			if(parent.tabPosition != CTabFolderNext.RIGHT) {
				gc.drawLine(x2, y1, x2, y2); // right
			}
			if(parent.tabPosition != CTabFolderNext.TOP) {
				gc.drawLine(x1, y1, x2, y1); // top
			}
			if(parent.tabPosition != CTabFolderNext.BOTTOM) {
				gc.drawLine(x1, y2, x2, y2); // bottom
			}
		}
	}

	private void drawClientArea(GC gc, Point size, int borderLeft, int borderRight,
			int borderTop, int borderBottom, int highlightHeader, int highlightMargin) {
		gc.setBackground(parent.getBackground());
		int xClient = borderLeft  + highlightMargin;
		int yClient = borderTop ;
		if (parent.tabPosition == CTabFolderNext.TOP) {
			yClient += parent.tabHeight + highlightHeader;
		} else {
			yClient += highlightMargin;
		}

		int width = size.x  - borderLeft - borderRight - highlightMargin;
		int height = size.y - borderTop - borderBottom - highlightMargin;
		if(isLeftOrRightTabPosition()) {
			width -= (parent.tabWidth + highlightHeader);
		} else {
			height -= (parent.tabHeight  + highlightHeader);
		}

		gc.fillRectangle(xClient, yClient, width, height);
	}

	private void drawHighlightMargin(GC gc, Point size, int borderLeft, int borderRight,
			int borderTop, int borderBottom, int highlightHeader, int highlightMargin) {
		int[] shape = null;
		switch(parent.tabPosition) {
		case CTabFolderNext.LEFT:
			shape = createLeftHighlightShape(size, borderLeft, borderRight, borderTop,
					borderBottom, highlightHeader, highlightMargin);
			break;
		case CTabFolderNext.RIGHT:
			shape = createRightHighlightShape(size, borderLeft, borderRight, borderTop,
					borderBottom, highlightHeader, highlightMargin);
			break;
		case CTabFolderNext.BOTTOM:
			shape = createBottomHighlightShape(size, borderLeft, borderRight, borderTop,
					borderBottom, highlightHeader, highlightMargin);
			break;
		default:
			shape = createTopHighlightShape(size, borderLeft, borderRight, borderTop, borderBottom,
					highlightHeader, highlightMargin);
			break;
		}
		// If horizontal gradient, show gradient across the whole area
		if (parent.selectedIndex != -1 && parent.selectionGradientColors != null && parent.selectionGradientColors.length > 1 && !parent.selectionGradientVertical) {
			drawBackground(gc, shape, true);
		} else if (parent.selectedIndex == -1 && parent.gradientColors != null && parent.gradientColors.length > 1 && !parent.gradientVertical) {
			drawBackground(gc, shape, false);
		} else {
			gc.setBackground(parent.selectedIndex != -1 && parent.shouldHighlight() ? parent.selectionBackground : parent.getBackground());
			gc.fillPolygon(shape);
		}
	}

	private int[] createTopHighlightShape(Point size, int borderLeft, int borderRight, int borderTop,
			int borderBottom, int highlightHeader, int highlightMargin) {
		int x1 = borderLeft;
		int y1 = borderTop + parent.tabHeight + highlightHeader;
		int x2 = size.x - borderRight;
		int y2 = size.y - borderBottom;
		return new int[] {x1,y1, x1+highlightMargin,y1, x1+highlightMargin,y2-highlightMargin,
						   x2-highlightMargin,y2-highlightMargin, x2-highlightMargin,y1,
						   x2,y1, x2,y2, x1,y2};
	}

	private int[] createLeftHighlightShape(Point size, int borderLeft, int borderRight, int borderTop,
			int borderBottom, int highlightHeader, int highlightMargin) {
		int x1 = borderLeft + parent.tabWidth + highlightHeader;
		int y1 = borderTop;
		int x2 = size.x - borderRight;
		int y2 = size.y - borderBottom;
		return new int[] {x1,y1, x2,y1, x2,y2, x1,y2, x1,y2-highlightMargin, x2-highlightMargin,y2-highlightMargin,
				x2-highlightMargin,y1+highlightMargin, x1,y1+highlightMargin};
	}

	private int[] createRightHighlightShape(Point size, int borderLeft, int borderRight, int borderTop,
			int borderBottom, int highlightHeader, int highlightMargin) {
		int x1 = borderLeft;
		int y1 = borderTop;
		int x2 = size.x - parent.tabWidth - highlightHeader - borderRight;
		int y2 = size.y - borderBottom;
		return new int[] {x1,y1, x2,y1, x2,y1+highlightMargin, x1+highlightMargin,y1+highlightMargin,
				x1+highlightMargin,y2-highlightMargin, x2,y2-highlightMargin, x2,y2, x1,y2};
	}


	private int[] createBottomHighlightShape(Point size, int borderLeft, int borderRight, int borderTop,
			int borderBottom, int highlightHeader, int highlightMargin) {
		int x1 = borderLeft;
		int y1 = borderTop;
		int x2 = size.x - borderRight;
		int y2 = size.y - borderBottom - parent.tabHeight  - highlightHeader;
		return new int[] {x1,y1, x2,y1, x2,y2, x2-highlightMargin,y2,
						   x2-highlightMargin, y1+highlightMargin, x1+highlightMargin,y1+highlightMargin,
						   x1+highlightMargin,y2, x1,y2};
	}
	...
	private static void drawRoundRectangle(GC gc, Rectangle chevronRect) {
	...
	private static void drawChevronContent(GC gc, int x, int y, String chevronString) {
	...
	void drawLeftOrientedUnselectedBorder(GC gc, Rectangle bounds){
		int[] top  = parent.simple ? SIMPLE_TOP_LEFT_CORNER : TOP_LEFT_CORNER;
		int[] bottom  = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER : BOTTOM_LEFT_CORNER;
		int[]  shape = new int[top.length + bottom.length + 4];
		int x = bounds.x;
		int y = bounds.y;
		int height = bounds.height;
		...
		int index = 0;
		shape[index++] = x + width - 1;
		shape[index++] = y;

		for (int i = top.length/2 - 1; i >= 0; i--) {
			shape[index++] = x + top[2*i];
			shape[index++] = y + top[2*i+1];
		}
		for (int i = 0; i < bottom.length/2; i++) {
			shape[index++] = x + bottom[2*i];
			shape[index++] = y + height + bottom[2*i+1];
		}
		shape[index++] = x + width - 1;
		shape[index] = y + height;
	...
	void drawRightOrientedUnselectedBorder(GC gc, Rectangle bounds){
		int[] top  = parent.simple ? SIMPLE_TOP_RIGHT_CORNER : TOP_RIGHT_CORNER;
		int[] bottom  = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER : BOTTOM_RIGHT_CORNER;
		int[]  shape = new int[top.length + bottom.length + 4];
		int x = bounds.x;
		int y = bounds.y;
		int height = bounds.height;
		...
		for (int i = 0; i < top.length / 2; i++) {
			shape[index++] = x + width + top[2*i] - 1;
			shape[index++] = y + top[2*i+1];
		}
		for (int i = bottom.length/2 - 1; i >= 0 ; i--) {
			shape[index++] = x + width + bottom[2*i] -1;
			shape[index++] = y + height + bottom[2*i+1];
		}
		shape[index++] = x;
		shape[index] = y + height;
	...
	void drawSelected(int itemIndex, GC gc, Rectangle bounds, int state ) {
		CTabItemNext item = parent.items[itemIndex];
		...
		if ((state & SWT.BACKGROUND) != 0) {
			int[] shape;
			drawSelectionHighlight(gc, borderLeft, borderRight, borderTop, borderBottom, size);

			if (parent.single && !isLeftOrRightTabPosition()) {
				...
				switch (parent.tabPosition) {
					case CTabFolderNext.BOTTOM:
						...
						break;
					case CTabFolderNext.LEFT:
						shape = createSelectedShapeForLeftOrientedTabs(bounds, size.y);
						break;
					case CTabFolderNext.RIGHT:
						shape = createSelectedShapeForRightOrientedTabs(bounds, size.y);
						break;
					default:
		...
		if ((state & SWT.FOREGROUND) != 0) {
			...
			xDraw += drawImage(gc, item, xDraw, y, height, rightEdge, trim);
	...
	private int drawImage(GC gc, CTabItemNext item, int x, int y, int height, int rightEdge, Rectangle trim) {
		Image image = item.getImage();
		if (image != null && !image.isDisposed()) {
			Rectangle imageBounds = image.getBounds();
			// only draw image if it won't overlap with close button
			int maxImageWidth = rightEdge - x - (trim.width + trim.x);
			...
			if (imageBounds.width < maxImageWidth) {
				...
				return imageBounds.width + INTERNAL_SPACING;
			}
		}
		return 0;
	}

	private void drawSelectionHighlight(GC gc, int borderLeft, int borderRight, int borderTop, int borderBottom,
			Point size) {
		...
		int hh;
		if(isLeftOrRightTabPosition()) {
			xx = (parent.tabPosition == CTabFolderNext.LEFT) ?
					borderLeft + parent.tabWidth + 1 : size.x - borderRight - parent.tabWidth - highlightHeader;
			yy = borderTop + 1;
			ww = highlightHeader - 1;
			hh = size.y - borderTop - borderBottom - 1;
		} else {
			xx = borderLeft;
			yy = parent.onBottom ? size.y - borderBottom - parent.tabHeight - highlightHeader : borderTop + parent.tabHeight + 1;
			ww = size.x - borderLeft - borderRight;
			hh = highlightHeader - 1;
		}
	...
	private boolean isLeftOrRightTabPosition() {
		return parent.tabPosition == CTabFolderNext.LEFT || parent.tabPosition == CTabFolderNext.RIGHT;
	}

	private int[] createSelectedShapeForLeftOrientedTabs(Rectangle bounds, int fullHeight) {
		int[] top  = parent.simple ? SIMPLE_TOP_LEFT_CORNER : TOP_LEFT_CORNER;
		int[] bottom  = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER : BOTTOM_LEFT_CORNER;

		int[]  shape = new int[top.length + bottom.length + 8];
		int index = 0;
		shape[index++] = bounds.x + bounds.width;
		shape[index++] = 0;
		shape[index++] = shape[0];
		shape[index++] = bounds.y;

		for (int i = top.length/2 - 1; i >= 0; i--) {
			shape[index++] = bounds.x + top[2*i];
			shape[index++] = bounds.y + top[2*i+1];
		}
		for (int i = 0; i < bottom.length/2; i++) {
			shape[index++] = bounds.x + bottom[2*i];
			shape[index++] = bounds.y + bounds.height + bottom[2*i+1];
		}
		shape[index++] = shape[0];
		shape[index++] = bounds.y + bounds.height;
		shape[index++] = shape[0];
		shape[index] = fullHeight - 1;
		return shape;
	}

	private int[] createSelectedShapeForRightOrientedTabs(Rectangle bounds, int fullHeight) {
		int[] top  = parent.simple ? SIMPLE_TOP_RIGHT_CORNER : TOP_RIGHT_CORNER;
		int[] bottom  = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER : BOTTOM_RIGHT_CORNER;

		int[]  shape = new int[top.length + bottom.length + 8];
		int index = 0;
		shape[index++] = bounds.x +1;
		shape[index++] = 0;
		shape[index++] = bounds.x +1;
		shape[index++] = bounds.y;

		for (int i = 0; i < top.length/2; i++) {
			shape[index++] = bounds.x + bounds.width + top[2*i] - 1;
			shape[index++] = bounds.y + top[2*i+1];
		}
		for (int i = bottom.length/2 - 1; i >= 0 ; i--) {
			shape[index++] = bounds.x + bounds.width + bottom[2*i] - 1;
			shape[index++] = bounds.y + bounds.height + bottom[2*i+1];
		}
		shape[index++] = bounds.x +1;
		shape[index++] = bounds.y + bounds.height;
	...
	void drawTabArea(GC gc, Rectangle bounds, int state) {
		...
		if (tabHeight == 0) {
			...
			int x1 = borderLeft - 1;
			int x2 = size.x - borderRight;
			int y1 = parent.onBottom ? size.y - borderBottom - highlightHeader - 1 : borderTop + highlightHeader;
		...
		int x = parent.onRight ? size.x - borderRight - tabWidth : Math.max(0, borderLeft - 1);
		int y = parent.onBottom ? size.y - borderBottom - tabHeight : borderTop;
		int width;
		int height;
		if(isLeftOrRightTabPosition()) {
			width = tabWidth - 1;
			height= size.y - borderTop - borderRight + 1;
		} else {
			width = size.x - borderLeft - borderRight + 1;
			height = tabHeight - 1;
		}

		switch (parent.tabPosition) {
		case CTabFolderNext.BOTTOM:
			shape = createTabHeaderBottom(style, borderLeft, highlightHeader, x, y, width, height);
			break;
		case CTabFolderNext.LEFT:
			shape = createTabHeaderLeft(style, highlightHeader, x, y, width, height);
			break;
		case CTabFolderNext.RIGHT:
			shape = createTabHeaderRight(style, x, y, width, height);
			break;
		default:
			shape = createTabHeaderTop(style, highlightHeader, x, y, width, height);
			break;
		}

		// Fill in background
		boolean bkSelected = parent.single && selectedIndex != -1 && !isLeftOrRightTabPosition();
	...
	private int[] createTabHeaderTop(int style, int highlightHeader, int x, int y, int width, int height) {
		int[] left;
		int[] right;
		if ((style & SWT.BORDER) != 0) {
			left = parent.simple ? SIMPLE_TOP_LEFT_CORNER : TOP_LEFT_CORNER;
			right = parent.simple ? SIMPLE_TOP_RIGHT_CORNER : TOP_RIGHT_CORNER;
		} else {
			left = parent.simple ? SIMPLE_TOP_LEFT_CORNER_BORDERLESS : TOP_LEFT_CORNER_BORDERLESS;
			right = parent.simple ? SIMPLE_TOP_RIGHT_CORNER_BORDERLESS : TOP_RIGHT_CORNER_BORDERLESS;
		}
	...
	private int[] createTabHeaderLeft(int style, int highlightHeader, int x, int y, int width, int height) {
		int[] bottom;
		int[] top;
		if ((style & SWT.BORDER) != 0) {
			bottom = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER : BOTTOM_LEFT_CORNER;
			top = parent.simple ? SIMPLE_TOP_LEFT_CORNER : TOP_LEFT_CORNER;
		} else {
			bottom = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER_BORDERLESS : BOTTOM_LEFT_CORNER_BORDERLESS;
			top = parent.simple ? SIMPLE_TOP_LEFT_CORNER_BORDERLESS : TOP_LEFT_CORNER_BORDERLESS;
		}
		int[] shape = new int[bottom.length + top.length + 4];
		int index = 0;
		shape[index++] = x + width + highlightHeader + 1;
		shape[index++] = y + height - 1;
		for (int i = bottom.length/2 - 1; i >= 0; i--) {
			shape[index++] = x + bottom[2*i];
			shape[index++] = shape[1] + bottom[2*i+1];
		}
		for (int i = 0; i < top.length/2; i++) {
			shape[index++] = x + top[2*i];
			shape[index++] = y + top[2*i+1];
	...
	private int[] createTabHeaderRight(int style, int x, int y, int width, int height) {
		int[] bottom;
		int[] top;
		if ((style & SWT.BORDER) != 0) {
			bottom = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER : BOTTOM_RIGHT_CORNER;
			top = parent.simple ? SIMPLE_TOP_RIGHT_CORNER : TOP_RIGHT_CORNER;
		} else {
			bottom = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER_BORDERLESS : BOTTOM_RIGHT_CORNER_BORDERLESS;
			top = parent.simple ? SIMPLE_TOP_RIGHT_CORNER_BORDERLESS : TOP_RIGHT_CORNER_BORDERLESS;
		}
		int[] shape = new int[bottom.length + top.length + 4];
		int endX = x + width;
		int index = 0;
		...
		shape[index++] = y;
		for (int i = 0; i < top.length/2; i++) {
			shape[index++] = endX + top[2*i];
			shape[index++] = y + top[2*i+1];
		}
		for (int i = bottom.length/2 - 1; i >= 0 ; i--) {
			shape[index++] = endX + bottom[2*i];
			shape[index++] = y + height + bottom[2*i+1] - 1;
		}
		shape[index++] = x;
		shape[index] = y + height - 1;
		return shape;
	...
	private int[] createTabHeaderBottom(int style, int borderLeft, int highlightHeader, int x, int y, int width,
			int height) {
		int[] left;
		int[] right;
		if ((style & SWT.BORDER) != 0) {
			left = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER : BOTTOM_LEFT_CORNER;
			right = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER : BOTTOM_RIGHT_CORNER;
		} else {
			left = parent.simple ? SIMPLE_BOTTOM_LEFT_CORNER_BORDERLESS : BOTTOM_LEFT_CORNER_BORDERLESS;
			right = parent.simple ? SIMPLE_BOTTOM_RIGHT_CORNER_BORDERLESS : BOTTOM_RIGHT_CORNER_BORDERLESS;
		}
		int[] shape = new int[left.length + right.length + 4];
		int index = 0;
		shape[index++] = x;
	...
	void drawUnselected(int index, GC gc, Rectangle bounds, int state) {
		CTabItemNext item = parent.items[index];
		int x = bounds.x;
		int y = bounds.y;
		int height = bounds.height;
		int width = bounds.width;
		...
		if ((state & SWT.BACKGROUND) != 0) {
			switch(parent.tabPosition) {
			case CTabFolderNext.LEFT:
				drawLeftOrientedUnselectedBorder(gc, bounds);
				break;
			case CTabFolderNext.RIGHT:
				drawRightOrientedUnselectedBorder(gc, bounds);
				break;
			default:
		...
		if ((state & SWT.FOREGROUND) != 0) {
			...
			if (image != null && !image.isDisposed() && parent.showUnselectedImage) {
				...
				if (imageBounds.width < maxImageWidth) {
					int imageY = y + (height - imageBounds.height) / 2;
					imageY += parent.onBottom ? -1 : 1;
					gc.drawImage(image, xDraw, imageY);
